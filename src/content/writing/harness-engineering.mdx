...
Here is a claim that took me a while to actually believe: the model is not the agent. The agent is the model plus a pile of ordinary software around it, and most of what separates a demo from something you would trust to run unattended lives in that software, not in the weights.

The community has finally given the pile a name. **Agent = Model + Harness.** The [reference framing](https://en.wikipedia.org/wiki/Agent_harness) puts it plainly: the harness is "the software infrastructure surrounding a large language model that enables it to operate as an AI agent." DJ Farrelly has the one-liner I keep coming back to: in every engineering discipline a harness is "the layer that connects, protects, and orchestrates components, without doing the work itself."

I did not arrive at this from theory. I spent months building one (a control-plane CLI wrapped around an open-source agent runtime), and I work on agent infrastructure day to day. Every hard bug, every "why is it burning $40 and looping," every "the eval passed but the CLI is broken," turned out to be a harness problem, not a model problem. So this is the post I wish I had read first: what the harness is, why it matters more than it looks, and where the actual engineering is.
...
Cognition adds a rule that cost me a real bug before I internalized it: share full traces, not stray messages, and be very careful with parallel sub-agents, because "actions carry implicit decisions, and conflicting decisions carry bad results." Two sub-agents that cannot see each other's context will make locally reasonable, globally incompatible choices, and you will spend an afternoon figuring out why the output contradicts itself.
...
I believe this strongly enough that I spent months building a harness to feel it, not just read about it: a control plane wrapped around one of these runtimes, with routing, layered memory, budgets, evals, traces, and a genuinely hostile testing pass that found bugs no green test suite ever would. That is the next post. This one was the map. The next one is the territory.
