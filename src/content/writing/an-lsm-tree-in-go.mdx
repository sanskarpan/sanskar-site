...
The default is 10 bits per key, which puts the false-positive rate near 1%. That is the classic trade curve: more bits per key means fewer false positives and more memory. The relationship is roughly the following, and it is mathematical, not measured on this engine:

<Bars label="Fig 5" caption="Bloom false-positive rate versus bits per key, at the optimal number of hash functions. These are the theoretical rates for the filter design, not benchmarks of this engine. The default is 10 bits per key." bars={[
...
## Design targets, not measured guarantees

The README carries a set of **design targets** for modern SSD hardware. I want to be precise about the label: these are the numbers the engine is built to hit, not benchmark results checked in to the repo. The benchmark harness exists (`make bench`, with `BenchmarkWALAppend`, `BenchmarkSkipListInsert`, `BenchmarkEngineSequentialWrite` and others), but the results table in the README is still a template, so I am not going to quote a throughput I cannot show you the measurement for.
...
<Mermaid label="Fig 9" caption="The gateway writes through a cluster facade, never the raw engine. In clustered mode, writes go through Raft consensus and each node applies committed commands into its own local LSM engine." chart={`
flowchart TB
...
The [full engine is on GitHub](https://github.com/sanskarpan/lsm-tree-storage), including the 7-panel real-time dashboard that renders every event described here (WAL appends, Bloom checks, flushes, compactions) as it happens.
