...
The default is 10 bits per key, which puts the false-positive rate near 1%. That is the classic trade curve: more bits per key means fewer false positives and more memory. The relationship is the following, and the engine's empirical rates in `docs/benchmarks.md` track it closely, about 8% at 6 bits, 1% at the default 10, and 0.3% at 14:

<Bars label="Fig 5" caption="Bloom false-positive rate versus bits per key, at the optimal number of hash functions. This is the theoretical curve for the filter design; the engine's measured rates match it closely. The default is 10 bits per key." bars={[
  { label: "6 bits/key", value: 5.6, display: "~5.6%" },
  { label: "8 bits/key", value: 2.1, display: "~2.1%" },
  { label: "10 bits/key", value: 0.8, display: "~0.8%", accent: true },
  { label: "12 bits/key", value: 0.3, display: "~0.3%" },
]} max={6} />
